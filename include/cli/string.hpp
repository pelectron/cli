...
public:
  ...
  constexpr View(const View &o) noexcept : str_(o.str_), size_(o.size_) {}
  constexpr View(View &&o) noexcept : str_(o.str_), size_(o.size_) {}

  template <typename Ch>
    requires std::convertible_to<Ch *, CharType *>
  constexpr View(View<Ch> other) : str_(other.str_), size_(other.size_) {}

  constexpr View(value_type *str, std::size_t size) noexcept
      : str_(str), size_(size) {}

  constexpr View(value_type *str) noexcept : str_(str), size_(0) {
    while (str_[size_] != 0)
      ++size_;
  }
  ...
  constexpr View(Char (&array)[Size]) noexcept : str_(array), size_(Size) {}
  ...
  constexpr bool operator<(const View &other) const noexcept {
    ...
    // substrings are equal-> if this is shorter than other, this is "less than"
    // other
  ...
  template <typename Ch> constexpr bool starts_with(View<Ch> s) const noexcept {
  ...
  constexpr std::size_t find_first_not_of(value_type c,
                                          std::size_t pos = 0) const noexcept {
    for (std::size_t i = pos; i < size_; ++i)
      if (c != str_[i])
        return i;
  ...
  template <typename Ch>
  constexpr std::size_t find_first_not_of(View<Ch> s,
                                          std::size_t pos = 0) const noexcept {
    for (std::size_t i = pos; i < size_; ++i) {
      std::size_t cnt{0};
      for (std::size_t j = 0; j < s.size_; ++j) {
  ...
  constexpr std::size_t
  find_last_not_of(const value_type *c, std::size_t pos = npos) const noexcept {
  ...
  constexpr std::size_t find(value_type c, std::size_t pos = 0) const noexcept {
    for (std::size_t i = pos; i < size_; ++i)
      if (c == str_[i])
        return i;
    return npos;
...
template <typename CharT, std::size_t N> struct StringLiteral {
  ...
  template <CharT... Cs>
  constexpr StringLiteral(string_constant<CharT, Cs...>) : s{Cs..., 0} {}
...
template <typename T> inline constexpr bool is_string_constant_v = false;
