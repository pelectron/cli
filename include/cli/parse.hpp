...
template <traits::Character T, typename CharT> class Char {
public:
  constexpr ParseResult<T, CharT> operator()(View<const CharT> str) const {
    if (str.size() == 0)
      return Error::too_few_characters;
 ...
 * @tparam T
 */
template <traits::String T, typename CharT> class String {
public:
  constexpr ParseResult<T, CharT> operator()(View<const CharT> str) const {
    if (str.size() == 0)
      return Error::too_few_characters;
...
public:
  constexpr ParseResult<T, CharT> operator()(View<const CharT> str) const {
    ...
    if constexpr ((Format & Fmt::normal) == Fmt::normal) {
      ...
      const auto ret = [&int_res, &frac, &str,
                        &negative]() -> ParseResult<T, CharT> {
        if constexpr (not traits::is_signed) {
          (void)negative;
...
template <traits::Enum T, typename CharT, bool AllowNumbers> class Enum {
public:
  constexpr ParseResult<T, CharT> operator()(View<const CharT> str) const {
...
template <class T, typename CharT> class DefaultParse {
public:
...
template <typename CharT> class DefaultParse<bool, CharT> {
  static constexpr View<const CharT> truthy[]{"true", "TRUE", "1", "yes", "y"};
  static constexpr View<const CharT> falsy[]{"false", "FALSE", "0", "no", "n"};
...
template <typename CharT, CharT Assignment = '=', CharT MemberSeparator = ',',
          CharT Prefix = '{', CharT Postfix = '}', class... Fields>
class FieldGroup {
...
struct field_parser_for {
  ...
  template <typename F> using tpf = to_parse_field<F, CharT>;

  using type = type_list::apply_t<
      type_, type_list::transform_t<tpf, typename ctti::TypeInfo<T>::fields>>;
};
...
template <typename CharT> class NullParse {
public:
  constexpr ParseResult<dummy, CharT> operator()(View<const CharT> str) const {
    return {dummy{}, str};
  }
};
...
Error parse_args_impl(Tuple &t, View<const CharT> v,
