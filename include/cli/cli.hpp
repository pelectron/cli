...
namespace cli {
   ...
   */
  template<Config Cfg,
           cli::Output<typename Cfg::char_type> Out,
           Command... Commands>
  class Cli {
  public:
    using config = Cfg;
    ...
    template<Config Cfg_,
             cli::Output<typename Cfg_::char_type> Out_,
             cli::Command... Cmds>
    constexpr Cli(Cfg_, Out_ &&out, Cmds &&...cmds)
      : commands_{Cfg_{}, std::forward<Commands>(cmds)...},
        out_(std::forward<Out_>(out)),
  ...
  template<Config Cfg,
           cli::Output<typename Cfg::char_type> Out,
           Command... Commands>
  Cli(Cfg &&, Out &&, Commands &&...) -> Cli<std::remove_cvref_t<Cfg>,
