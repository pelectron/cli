...
namespace cli {
  ...
  template<typename T>
  struct get_stream_char_type;

  template<typename Char>
  struct get_stream_char_type<Error (*)(Char)> {
    using type = std::remove_const_t<Char>;
  };

  template<typename Char>
  struct get_stream_char_type<Error (*)(Char) noexcept> {
    using type = std::remove_const_t<Char>;
  };
   ...
   */
  template<Config Cfg, BasicOutputStream Stream>
  class AnsiOutput {
