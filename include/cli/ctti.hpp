...
// clang-format off
  ...
  #define CTTI_TYPE_PRETTY_FUNCTION_PREFIX "cli::CharView cli::ctti::dtl::name_impl() [T = "
  #define CTTI_TYPE_PRETTY_FUNCTION_SUFFIX "]"
...
template <typename T, typename CharT = char> consteval auto name() {
...
[[nodiscard]] consteval auto member_name_impl() -> CharView {
  const auto name = CharView{std::source_location::current().function_name()};
#if defined(__clang__) and not defined(_MSC_VER)
...
template <class T, typename CharT = char> struct TypeInfo {
  using type = T;
