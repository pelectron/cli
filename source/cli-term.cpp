/**
 * @defgroup cli-term cli-term
 *
 * cli-term is a command line executable to connect to serial ports or ethernet
 * sockets to systems running a cli::Engine. You can compile and run this
 * executable on any system that [asio](https://think-async.com/Asio/) supports.
 *
 * ### Usage
 *
 * ``cli-term args...``
 ...
 * - -b, --baudrate: the serial port baudrate. A positive number. Default is
 *   115200.
 *
 * ## Example
 *
 ...
 *
 * ``cli-term --adress COM4``
 *
 * Connecting to a socket with 168.0.0.5 on port 80
 *
 * ``cli-term --adress 168.0.0.5:80``
 *
 * ### Building
 *
...
static void print_settings(const Settings &settings) {
  Term::cout << "Settings:\n";
  Term::cout << std::format(
                  "  address: {}\n  size: {}\n  delimiter: {}\n  endian: {}\n",
                  settings.address,
                  settings.char_size,
                  std::string_view{cli::ctti::enum_name(settings.delim).data()},
                  std::string_view{
                    cli::ctti::enum_name(settings.endian).data()})
             << std::flush;
...
static Settings get_settings(const std::vector<std::string_view> &args) {
  Settings settings{};
  ...
  for (std::size_t i = 0; i < (args.size() - 1); i += 2) {
    ...
    if (args[i] == "-a" or args[i] == "--address") {
      ...
      if (arg == "none") {
        ...
        Term::cerr << std::format("Invalid argument value for parity, expected "
      ...
      if (arg == "big") {
        ...
        Term::cerr << std::format("Invalid argument value for endian, expected "
      ...
      Term::cerr << std::format("unknown keyword argument '{}'\n", args[i]);
      print_usage();
      exit(-1);
    }
  }
...
static std::string to_utf8(char16_t cp) {
  char buf[16]{};
  std::mbstate_t state{};
...
public:
  ConnectionI(asio::io_context &ctx, const Settings &settings)
    : settings(settings), read_buf(256, 0), ctx(ctx) {}

  virtual ~ConnectionI() { close(); }
...
class Connection : public ConnectionI {
  Stream stream;
...
public:
  Connection(asio::io_context &ctx, const Settings &settings)
    : ConnectionI(ctx, settings), stream(ctx) {
    if constexpr (std::is_same_v<Stream, asio::serial_port>) {
      ...
      try {
        ep =
          asio::ip::tcp::endpoint(asio::ip::make_address(addr), port_res.value);
      } catch (...) {
        Term::cerr << std::format(
...
int main(int argc, const char **argv) {
  ...
  const Settings settings =
    get_settings(std::vector<std::string_view>{argv + 1, argv + argc});
  ...
  try {
    std::shared_ptr<ConnectionI> conn;
    if (settings.address.find(':') == std::string::npos) {
      conn = std::make_shared<Connection<asio::serial_port>>(ctx, settings);
    } else {
      conn = std::make_shared<Connection<asio::ip::tcp::socket>>(ctx, settings);
    }
    conn->start();
    return 0;
  } catch (const Term::Exception &re) {
    ...
    Term::cerr << std::format("Error: {}:{}", e.code().value(), e.what())
               << std::flush;
    return -1;
  } catch (const std::runtime_error &e) {
    Term::cerr << e.what() << std::flush;
    return -1;
  }
