...
template <class F> struct FixpointTestVector {
  F value;
  std::string input;
  std::string rest;
};
...
TEST_CASE("parse::FixPoint fixed<1,1>", "[parse][fixpoint]") {
  SECTION("signed") {
    REQUIRE(cli::parse::DefaultParse<psm::signed_fixed<1, 1>>{}("0")
                .value.value() == 0);
...
TEST_CASE("parse::FixPoint fixed<16,8>", "[parse][fixpoint]") {
  SECTION("signed") {
    REQUIRE(cli::parse::DefaultParse<psm::signed_fixed<16, 8>>{}("0")
                .value.value() == 0);
...
TEST_CASE("parse::FixPoint fixed<16,16>", "[parse][fixpoint]") {
  SECTION("signed") {
    ...
    REQUIRE(cli::parse::DefaultParse<psm::signed_fixed<16, 16>>{}("0.0")
                .value.value() == 0);
    CHECK(cli::parse::DefaultParse<psm::signed_fixed<16, 16>>{}(
              "32767.99998474121")
...
TEST_CASE("parse::FixPoint fixed<3,21>", "[parse][fixpoint]") {
  SECTION("signed") {
    REQUIRE(cli::parse::DefaultParse<psm::signed_fixed<3, 21>>{}("0")
                .value.value() == 0);
    REQUIRE(cli::parse::DefaultParse<psm::signed_fixed<3, 21>>{}("0.0")
                .value.value() == 0);
    CHECK(cli::parse::DefaultParse<psm::signed_fixed<3, 21>>{}(
              "3.999999523162842")
              .value.value() == 0x7fffffu);
